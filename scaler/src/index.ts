...
class LoadBalancer {
    ...
    private async checkInstanceHealth(instance: Instance): Promise<void> {
        return new Promise((resolve, reject) => {
            const startTime = Date.now();
            const req = http.get(
                `http://localhost:${instance.port}/health`,
                { timeout: 2000 },
                (res) => {
    ...
    private async healthCheckAll() {
        const healthPromises = this.instances.map(instance =>
            this.checkInstanceHealth(instance).catch(() => { }) // Ignore individual failures
        );
    ...
    private shouldScaleDown(): boolean {
        ...
        const avgLoad = healthy.reduce((sum, i) => sum + this.getInstanceLoad(i), 0) / healthy.length;
        const hasIdleInstances = healthy.some(i =>
            now - i.lastRequestTime > idleThreshold && i.activeRequests === 0
        );
    ...
    async initialize() {
        ...
        const spawnPromises = Array(config.minInstances).fill(0).map(() =>
